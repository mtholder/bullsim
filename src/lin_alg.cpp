...
// this code was taken from an earlier release of Mr Bayes that did the relaxed clock mcmc
//	most of the code is old dls, but mth has added some functions and slightly modified other
// functions to make them more C++ style
...

{
	...
	isComplex = NO;
	rc = EigenRealGeneral (n, qMatrix, eigenValues, eigvalsImag, eigvecs, iwork, dwork); // mainly swofford's code
	if (rc != NO_ERROR && rc == RC_COMPLEX_EVAL)
			isComplex = YES;
...

{
	...
	for (i=0; i < n; i++)
		...
		for (j=0; j < n; j++)
			...
			for (k = 0; k < n; k++)
				{
				Cwork[i][j] = Cadd(Cwork[i][j], Cmul(Ceigvecs[i][k], Cmul(CinverseEigvecs[k][j], Cexp(RCmul(rt, Complex(eigenValues[k], eigvalsImag[k]))))));
				}
			}
		}
	for (i=0; i < n; i++)
...
{	double *g,*ev,*ie,*evRow;
	register int i,j,k;
	evRow=*eigvecs;
	g=gexp;
	for (k=0; k < n; k++)
		*g++=exp(*eigenValues++ * rt );
	for (i=0; i < n; i++)
		{for (j=0; j < n; j++)
			{*p = 0.0;
			g=gexp;
	...
	//MTH version that uses stored eigvecs[i][k] * inverseEigvecs[k][j] and stored exp(eigenValues[k] * t * r)
	{	//j=0 is EXCLUDED
		register int			i, j, k;
		double		sum;
		register double *g;
	...
	//MTH version that uses stored eigvecs[i][k] * inverseEigvecs[k][j] and stored exp(eigenValues[k] * t * r)
	{	//j=0 is EXCLUDED
		register int			i, k,eimjump,eimsjump,pjump;
		double		sum;
		register double *g;
		g=gexp;
		for (k=0; k < n; k++)
	...
	void ChangeColumn (double rt, double *p, int n, double *eigenValues,  double *EigInvMult, double *gexp,int nonzero,double *preCalced,int onlycol)
	{
		...
		eimjump=nonzero*(onlycol-1);
		eimsjump=nonzero*(n-onlycol-1);
		pjump=n-onlycol;
		double *q;
		q=p;
		for (i=0; i < n; i++) {
	...
	{	//j=0 is EXCLUDED
		...
		double *q;
		p+=n*onlyrow; //advance p to the right row
		EigInvMult+=n*onlyrow*(n-1);
		q=p;
		p++; //iterate over the 0 colum
		for (j=1; j < n; j++)
			{g=gexp;
			sum = 0.0;
			for (k=0; k < n; k++)
	...
	//MTH version that uses stored eigvecs[i][k] * inverseEigvecs[k][j] and stored exp(eigenValues[k] * t * r)
	{
		...
		double *q;
		p+=onlyrow*n;
		EigInvMult+=nonzero*(n-1)*onlyrow;
		preCalced+=(n-1)*onlyrow;
		q=p;
		p++;
		for (j=1; j < n; j++)
			{g=gexp;
	...
	{	//j=0 is excluded
		...
		for (i=0; i < n; i++)
			{for (j=1; j < n; j++)
				{for (k=0; k < n; k++)
					//gEigInvMult[i][j][k] = (eigvecs[i][k] * inverseEigvecs[k][j]);
					*EigInvMult++ = (*eigvecs++) * inverseEigvecs[k][j];
				eigvecs-=n;
				}
			eigvecs+=n;
			}
...
#else
#	error CONDENSE_MATRICES must be defined
#endif
